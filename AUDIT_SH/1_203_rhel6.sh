...
echo -e "\n[53] Política MAC definida"
(sestatus 2>/dev/null | grep -q 'Loaded policy name' && echo "PASS" || echo "FAIL")

echo -e "\n[54] Serviços unconfined minimizados (Manual)"
...
echo -e "\n[56] Prelink removido"
(rpm -q prelink &>/dev/null && echo "FAIL" || echo "PASS")
...
(rpm -q bind &>/dev/null && echo "FAIL" || echo "PASS")

echo -e "\n[67] FTP server ausente"
(rpm -q vsftpd &>/dev/null && echo "FAIL" || echo "PASS")

echo -e "\n[68] HTTP server ausente"
(rpm -q httpd &>/dev/null && echo "FAIL" || echo "PASS")

echo -e "\n[69] IMAP/POP3 server ausente"
(rpm -q dovecot &>/dev/null && echo "FAIL" || echo "PASS")

echo -e "\n[70] Samba ausente"
(rpm -q samba &>/dev/null && echo "FAIL" || echo "PASS")

echo -e "\n[71] Proxy HTTP ausente"
(rpm -q squid &>/dev/null && echo "FAIL" || echo "PASS")

echo -e "\n[72] SNMP ausente"
(rpm -q net-snmp &>/dev/null && echo "FAIL" || echo "PASS")

echo -e "\n[73] NIS server ausente"
(rpm -q ypserv &>/dev/null && echo "FAIL" || echo "PASS")

echo -e "\n[74] Telnet server ausente"
(rpm -q telnet-server &>/dev/null && echo "FAIL" || echo "PASS")
...
(rpm -q rsh &>/dev/null && echo "FAIL" || echo "PASS")

echo -e "\n[81] Cliente talk ausente"
(rpm -q talk &>/dev/null && echo "FAIL" || echo "PASS")

echo -e "\n[82] Cliente telnet ausente"
(rpm -q telnet &>/dev/null && echo "FAIL" || echo "PASS")
...
echo -e "\n[103] Firewall com política default deny"
(systemctl is-active firewalld >/dev/null 2>&1 && ZONE=$(firewall-cmd --get-default-zone 2>/dev/null) && firewall-cmd --permanent --zone="$ZONE" --get-target 2>/dev/null | grep -Eq 'default|DROP') && echo "PASS" || echo "FAIL"

echo -e "\n[104] Apenas uma solução de firewall ativa"
([ "$(printf "%s\n" "$(systemctl is-active firewalld >/dev/null 2>&1 && echo F)" "$(systemctl is-active nftables >/dev/null 2>&1 && echo F)" "$(systemctl is-active iptables >/dev/null 2>&1 && echo F)" | grep -c F)" -eq 1 ]) && echo "PASS" || echo "FAIL"

echo -e "\n[105] Firewall ativo"
(systemctl is-active firewalld >/dev/null 2>&1) && echo "PASS" || echo "FAIL"

echo -e "\n[106] Zona padrão definida"
(firewall-cmd --get-default-zone >/dev/null 2>&1) && echo "PASS" || echo "FAIL"

echo -e "\n[107] Configuração permanente do firewall"
(systemctl is-active firewalld >/dev/null 2>&1 && firewall-cmd --permanent --zone=$(firewall-cmd --get-default-zone) --get-target >/dev/null 2>&1) && echo "PASS" || echo "FAIL"

echo -e "\n[108] TLS mínimo 1.2/1.3"
(update-crypto-policies --show 2>/dev/null | grep -qx FUTURE) && echo "PASS" || echo "FAIL"
#FUTURE impõe: TLS ≥ 1.2 cipher fortes SHA1 removido DH forte
...
( grep -Eq 'pam_pwquality\.so' /etc/pam.d/system-auth /etc/pam.d/password-auth 2>/dev/null && grep -Psiq '^\s*minlen\s*=\s*(1[4-9]|[2-9][0-9]+)\b' /etc/security/pwquality.conf /etc/security/pwquality.conf.d/*.conf 2>/dev/null && grep -Psiq '^\s*minclass\s*=\s*[4-9]\b' /etc/security/pwquality.conf /etc/security/pwquality.conf.d/*.conf 2>/dev/null ) && echo "PASS" || echo "FAIL"

echo -e "\n[132] pam_faillock habilitado"
(grep -Eq 'pam_faillock\.so' /etc/pam.d/system-auth /etc/pam.d/password-auth 2>/dev/null) && echo "PASS" || echo "FAIL"

echo -e "\n[133] pam_tally2 NÃO deve existir em RHEL 8/9"
! grep -Eq 'pam_tally2\.so' /etc/pam.d/*auth && echo "PASS" || echo "FAIL"

echo -e "\n[134] pam_pwhistory habilitado"
(grep -Eq 'pam_pwhistory\.so' /etc/pam.d/system-auth /etc/pam.d/password-auth 2>/dev/null) && echo "PASS" || echo "FAIL"
...
(grep -Eq 'pam_pwhistory\.so.*remember=([5-9]|[1-9][0-9]+)' /etc/pam.d/system-auth /etc/pam.d/password-auth 2>/dev/null) && echo "PASS" || echo "FAIL"
...
grep -q 'PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin' /root/.bash_profile && echo "PASS" || echo "FAIL"

echo -e "\n[153] umask 027"
grep -Rqs 'umask 027' /etc/profile /etc/bashrc /etc/profile.d 2>/dev/null && echo "PASS" || echo "FAIL"

echo -e "\n[154] TMOUT 900"
grep -Rqs 'TMOUT=900' /etc/profile /etc/bashrc /etc/profile.d 2>/dev/null && echo "PASS" || echo "FAIL"
...
echo -e "\n[160] su restrito ao wheel"
grep -q 'pam_wheel.so' /etc/pam.d/su && echo "PASS" || echo "FAIL"
...
( rpm -q rsyslog &>/dev/null && systemctl is-active rsyslog &>/dev/null ) && echo "PASS" || echo "FAIL"

echo -e "\n[164] Logrotate para rsyslog"
( grep -Eq 'rsyslog' /etc/logrotate.d/* 2>/dev/null ) && echo "PASS" || echo "FAIL"

echo -e "\n[165] auditd instalado e rodando"
( rpm -q audit audit-libs &>/dev/null && systemctl is-active auditd &>/dev/null ) && echo "PASS" || echo "FAIL"
...
echo -e "\n[172] Auditoria de eventos MAC"
auditctl -l 2>/dev/null | grep -Ei 'mac.*policy' >/dev/null && echo "PASS" || echo "FAIL"
...
( rpm -q aide &>/dev/null && [ -f /var/lib/aide/aide.db.gz ] ) && echo "PASS" || echo "FAIL"
...
stat -Lc "%U %G" /sbin/auditctl /sbin/aureport /sbin/ausearch /sbin/autrace /sbin/auditd /sbin/augenrules 2>/dev/null | awk '{if($1!="root" || $2!="root") exit 1}' && echo "PASS" || echo "FAIL"
...
( awk -F: '$3>=1000 && $1!="nfsnobody"{print $6}' /etc/passwd | while read -r h; do [ -d "$h" ] || exit 1; done ) && echo "PASS" || echo "FAIL"

echo -e "\n[200] Verificar ownership das homes dos usuários"
( awk -F: '$3>=1000 && $1!="nfsnobody"{print $1,$6}' /etc/passwd | while read -r u h; do [ -d "$h" ] && [ "$(stat -Lc %U "$h")" = "$u" ] || exit 1; done ) && echo "PASS" || echo "FAIL"

echo -e "\n[201] Verificar permissões das homes (<=750)"
( awk -F: '$3>=1000 && $1!="nfsnobody"{print $6}' /etc/passwd | while read -r h; do [ -d "$h" ] && [ "$(stat -Lc %a "$h")" -le 750 ] || exit 1; done ) && echo "PASS" || echo "FAIL"

echo -e "\n[202] Verificar dotfiles inseguros nas homes"
( while read -r h; do [ -d "$h" ] || continue; find "$h" -maxdepth 1 -type f -name ".*" -perm /022 2>/dev/null; done < <(awk -F: '$3>=1000 && $1!="nfsnobody"{print $6}' /etc/passwd) | grep -q . ) && echo "FAIL" || echo "PASS"

echo -e "\n[203] Verificar arquivos .forward/.netrc/.rhosts nas homes"
( while read -r h; do [ -d "$h" ] || continue; for f in "$h/.forward" "$h/.netrc" "$h/.rhosts"; do [ -e "$f" ] && echo "$f"; done; done < <(awk -F: '$3>=1000 && $1!="nfsnobody"{print $6}' /etc/passwd) | grep -q . ) && echo "FAIL" || echo "PASS"
